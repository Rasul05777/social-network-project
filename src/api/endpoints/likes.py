...
async def create_like(like: LikeCreate, user_id: int, db: AsyncSession=Depends(get_db)):
    return await add_like(db, like, user_id)


@router.get("/remove/{post_id}", summary="Удаление лайка")
async def delete_like( post_id:int, user_id: int, db: AsyncSession=Depends(get_db)):
    return await remove_like(db, post_id, user_id)


@router.get("/{post_id}/list/", response_model=int, summary="Все лайки на посте")
async def all_likes_post(post_id: int, db: AsyncSession=Depends(get_db)):
