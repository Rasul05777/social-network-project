...
async def post_create(post_create: PostCreate, user_id: int, db: AsyncSession=Depends(get_db)):
...
@router.get("/list/{author_id}", response_model=list[PostResponse], summary="Все посты пользователя")
async def get_all(author_id: int, db: AsyncSession=Depends(get_db)):
