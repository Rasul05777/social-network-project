...
async def get_user_id(user_id: int, current: UserResponse=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    return await get_user_by_id(db, user_id)


@router.get("/name/{user_name}", response_model=UserResponse, summary="Нахождение пользователя по имени")
async def get_user_name(user_name: str, current: UserResponse=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
